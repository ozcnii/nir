...
в том числе здания, сооружения, объекты незавершённого строительства [4].
...
данных со страниц веб-сайтов путём разбора их HTML-разметки [6].
...
может взаимодействовать с другой программой [7]. Применение API является
...
представления [8]. В задаче агрегации объявлений дедупликация необходима,
...
фотографий (perceptual hashing) [9].

### 1.1.6 Интеллектуальный консультант

В настоящей работе под интеллектуальным консультантом понимается
программный компонент, принимающий пользовательский запрос на
естественном языке и возвращающий ответ на основе ограниченного корпуса
проиндексированных данных. В качестве корпуса выступает база собранных
объявлений, а в качестве механизма обработки запроса – большая языковая
модель (Large Language Model, LLM), работающая по схеме генерации с
дополнением из базы знаний (Retrieval-Augmented Generation, RAG) [2].

Такой подход позволяет пользователю формулировать запрос вида «двухкомнатная
квартира в Самаре рядом с парком, до 6 миллионов, с ремонтом» и получать
релевантные объявления без использования традиционных полей-фильтров.
...
«Авито Недвижимость» – раздел крупнейшего в России сервиса объявлений
«Авито», посвящённый сделкам с жилой и коммерческой недвижимостью [10].
...
- наибольшая в России база объявлений по жилой и коммерческой
  недвижимости, охватывающая все регионы Российской Федерации;
...
- развитый поиск на карте с гео-фильтрами и отображением плотности
...
- отсутствие публичного программного интерфейса – API предоставляется
  только зарегистрированным профессиональным участникам;
...
недвижимости, существующий с 2001 года [11]. Позиционируется как
...
На рисунке 2 представлена страница поисковой выдачи сервиса «Циан».

![Рисунок 2 – Поисковая выдача сервиса Циан](screenshots/cian.png)
...
кредитованию [12]. На площадке размещаются объявления от частных лиц,
...
- обязательная юридическая проверка объектов, снижающая риск
  покупателя;
...
входящий в экосистему компании «Яндекс» [13]. Интегрирован с картами,
...
брокеров и застройщиков [14]. Объявления публикуются зарегистрированными
...
- низкая доля частных продавцов из-за ориентации на профессиональных
  участников;
- устаревший пользовательский интерфейс веб-версии.

На рисунке 5 представлена страница поиска сервиса Restate.

![Рисунок 5 – Страница поиска сервиса Restate](screenshots/restate.png)
...
| -------------------------------------- | :---: | :--: | :-----: | :----: | :-----: |
| Высокая доля объявлений от частных     |   +   |  −   |    −    |   −    |    −    |
| лиц                                    |       |      |         |        |         |
| Строгая модерация и низкая доля        |   −   |  +   |    +    |   +    |    +    |
| дублирующих объявлений                 |       |      |         |        |         |
| Развитая аналитика рынка (индексы и    |   −   |  +   |    ±    |   +    |    +    |
| графики динамики цен)                  |       |      |         |        |         |
| Публичный программный интерфейс для    |   −   |  ±   |    ±    |   −    |    −    |
| сторонних разработчиков                |       |      |         |        |         |
| Юридическое сопровождение сделок       |   −   |  −   |    +    |   ±    |    −    |
| Агрегация объявлений из нескольких     |   −   |  −   |    −    |   −    |    −    |
| площадок                               |       |      |         |        |         |
| Межсистемная дедупликация объявлений   |   −   |  −   |    −    |   −    |    −    |
| Поиск объявлений на естественном языке |   −   |  −   |    −    |   −    |    −    |
...
не реализует межсистемную дедупликацию объявлений и не поддерживает
поиск на естественном языке. Это обосновывает актуальность разработки
собственной системы.
...
максимальный охват массового сегмента частных объявлений («Авито»),
...
данных и предоставлять пользователю инструменты поиска, фильтрации и
получения рекомендаций в диалоговом режиме.
...
- индексирование корпуса объявлений для работы интеллектуального
  консультанта;
...
- поиск объявлений в режиме диалога с интеллектуальным консультантом
